...
public class FingerGripExercise extends Activity implements SensorEventListener {
	...
	*/	
	...
	public void onSensorChanged(SensorEvent event){
	    ...
	    if(!gameStart) {
	    	...
	    	//countDown.setText("" + ((Math.abs(x) < thresholdX)?"Y":"N") + ((Math.abs(deltaY) < thresholdY)?"Y":"N") + ((Math.abs(mLastZ - z) < thresholdZ)?"Y":"N"));
			if(Math.abs(x) < thresholdX && Math.abs(deltaY) < thresholdY && Math.abs(mLastZ - z) < thresholdZ) {
				stopped = true;
			} else {
				stopped = false;
				timer = System.currentTimeMillis();
